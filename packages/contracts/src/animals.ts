...
// ============================================================================
// M03 CLINICAL REMINDERS (T-106, RF09, §8/§9/§10, RNF07, RNF11). Additive.
// A background job (BullMQ+Redis) scans clinical events whose `nextDueDate` is
// upcoming/overdue and generates in-app reminders (the SOURCE OF TRUTH), then
// best-effort notifies via the NotificationPort (stub) with staggered retries
// (5min/30min/2h/24h). All timestamps are ISO-8601 UTC.
// ============================================================================

/** Reminder kind — mirrors the clinical event type that generated it. */
 ...
 * - `Pending`       — generated in-app, notification not yet sent
 * - `Sent`          — notification delivered via the NotificationPort
 * - `Failed`        — notification attempt failed; the reminder PERSISTS and
 *                     stays visible in-app (send is retried with backoff)
 * - `Acknowledged`  — a user marked it as attended
 * - `Dismissed`     — a user dismissed it
...
/**
 * An in-app clinical reminder (RF09) — the SOURCE OF TRUTH. Generated by the
 * background worker from a clinical event's `nextDueDate`. `clinicalEventId` is
 * the LOGICAL event id (stable across versions); the idempotency key
 * (clinicalEventId + dueDate + type) guarantees a re-scan never duplicates it.
 */
export interface ClinicalReminder {
  id: string;
  organizationId: string;
  animalId: string;
  /** Logical clinical event id that this reminder is for. */
  clinicalEventId: string;
  type: ReminderType;
  /** Due date (UTC) taken from the clinical event's `nextDueDate`. */
  dueDate: string;
  status: ReminderStatus;
  /** Notification channel used on the last send attempt (e.g. the stub driver). */
  channel?: string;
  /** ISO-8601 UTC. */
  createdAt: string;
  /** ISO-8601 UTC — when the notification was sent. */
  sentAt?: string;
  /** ISO-8601 UTC — when acknowledged/dismissed. */
  resolvedAt?: string;
  /** User who acknowledged/dismissed it. */
