...
| `GoogleSignInInput`, `CompleteProfileInput`, `IncompleteProfileError`, `PROFILE_REQUIRED_FIELDS`, campos `phone`/`documentId`/`address` en `AuthenticatedUser` | `src/auth.ts`     | @sebastian (core) | Publicado | T-Google-SignIn | Login con Google + gate de perfil incompleto (donar/apadrinar, adopción, voluntariado)                                                                                                      |
| `IdentityPort`, `IdentityClaims`, `FakeIdentityAdapter`                                                                                                        | `src/identity.ts` | @sebastian (core) | Publicado | T-Google-SignIn | Puerto simulable de verificación de identidad (Google Sign-In), mismo patrón que `PaymentPort`; adapter real (`GoogleIdentityAdapter`, google-auth-library) en `apps/api/src/core/identity` |
